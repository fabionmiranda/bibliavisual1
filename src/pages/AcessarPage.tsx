...
function getPreviewDiagramas(pt: boolean) {
  return [
    { label: pt ? 'Interlinear'      : 'Interlinear',        color: '#00d4ff' },
    { label: pt ? 'Quiástico'        : 'Chiastic',           color: '#ff2d55' },
    { label: pt ? 'Sintático'        : 'Syntactic',          color: '#00d4ff' },
    { label: pt ? 'Semântico'        : 'Semantic',           color: '#a855f7' },
    { label: pt ? 'Cristológico'     : 'Christological',     color: '#a855f7' },
    { label: pt ? 'Progressivo'      : 'Progressive',        color: '#ff2d55' },
    { label: pt ? 'Homilético'       : 'Homiletical',        color: '#00d4ff' },
    { label: pt ? 'Pastoral prático' : 'Practical Pastoral', color: '#a855f7' },
  ];
}

function getCards(pt: boolean) {
  const previewDiagramas = getPreviewDiagramas(pt);
  return [
    {
      id: 'o-que-sao',
      label: pt ? 'O que são os Diagramas?' : 'What are the Diagrams?',
      descricao: pt
        ...
        : 'Discover the 23 exegesis diagrams — a scientific and theological methodology that exhausts the biblical text through unique perspectives.',
      cta: pt ? 'Conhecer o método' : 'Learn the method',
      path: '/metodo',
      accentColor: '#a855f7',
        ...
        <div className="grid grid-cols-2 gap-1.5 mt-6">
          ...
          <div className="col-span-2 flex items-center justify-center px-2.5 py-1.5 rounded-lg border border-dashed border-white/10">
            <span className="text-[9px] font-bold text-white/25">
              {pt ? '+ 15 outros diagramas' : '+ 15 other diagrams'}
            </span>
          </div>
        </div>
      ),
    ...
    {
      id: 'veja-diagramas',
      label: pt ? 'Veja os Diagramas' : 'See the Diagrams',
      descricao: pt
        ? 'Acesse toda a biblioteca de livros bíblicos mapeados com os diagramas exegéticos — do Gênesis ao Apocalipse.'
        : 'Access the full library of biblical books mapped with exegetical diagrams — from Genesis to Revelation.',
      cta: pt ? 'Abrir biblioteca' : 'Open library',
      path: '/diagramas',
      accentColor: '#00d4ff',
      ...
      preview: (
        <div className="mt-6 space-y-2">
          {[
            { nome: pt ? 'Antigo Testamento' : 'Old Testament', qtd: pt ? '39 livros' : '39 books', cor: '#00d4ff' },
            { nome: pt ? 'Novo Testamento'   : 'New Testament', qtd: pt ? '27 livros' : '27 books', cor: '#ff2d55' },
          ].map(t => (
            <div
              key={t.nome}
          ...
          <div className="flex items-center justify-center gap-1.5 pt-1 opacity-40">
            ...
            <span className="text-[9px] font-bold text-white ml-0.5">
              {pt ? '23 perspectivas' : '23 perspectives'}
            </span>
          </div>
        </div>
      ),
    ...
    {
      id: 'tutoriais',
      label: pt ? 'Tutoriais' : 'Tutorials',
      descricao: pt
        ? 'Aprenda como interpretar e usar cada tipo de diagrama — passo a passo, de forma didática e acessível.'
        : 'Learn how to interpret and use each type of diagram — step by step, in a didactic and accessible way.',
      cta: pt ? 'Ver tutoriais' : 'See tutorials',
      path: '/tutoriais',
      accentColor: '#ff2d55',
      ...
      preview: (
        <div className="mt-6 space-y-2">
          {(pt
            ...
            : [
                'How to read a chiastic diagram',
                'Understanding the interlinear diagram',
                'Using the homiletical diagram',
                'Christological diagram in practice',
              ]
          ).map((t, i) => (
            <div
              key={i}
