...
export default function Navbar() {
  ...
  const linkCls = (active: boolean) => cn(
    'relative text-[11px] xl:text-xs font-black uppercase tracking-wider whitespace-nowrap transition-all duration-200 group px-2.5 py-1.5 xl:px-3',
        ...
        <div className="flex items-center justify-between h-[68px] lg:h-20">
          ...
          <div className="hidden md:flex items-center gap-0.5 lg:gap-1">
            ...
            {[
              ...
              <Link key={link.name} to={link.path} className={linkCls(isActive(link.path))}>
                {link.name}
                <span className={underline(isActive(link.path))} />
              </Link>
            ))}
            ...
            <Link
              to="/livraria"
              className={linkCls(isLibrariaActive)}
            >
              Livraria
              <span className={underline(isLibrariaActive)} />
            </Link>

            {/* Contatos */}
            <button
              onClick={scrollToFooter}
              className="relative text-[11px] xl:text-xs font-black uppercase tracking-wider whitespace-nowrap transition-all duration-200 group text-white/70 hover:text-white px-2.5 py-1.5 xl:px-3"
            >
              Contatos
            ...
            {/* Admin — subtle, smaller */}
            <Link
              to="/admin"
              className={cn(
                'relative text-[10px] xl:text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-all duration-200 group px-2 py-1.5',
                isActive('/admin') ? 'text-brand-rose' : 'text-white/30 hover:text-white/60'
              )}
            >
              Admin
            ...
            <div className="w-px h-4 bg-white/10 mx-1 lg:mx-2 shrink-0" />

            {/* Clube — hidden below xl */}
            <motion.a
              href="https://chat.whatsapp.com/HEIIFYAmLij62M0jRwwVhH"
              target="_blank" rel="noopener noreferrer"
              animate={{ opacity: [0.85, 1, 0.85], scale: [1, 1.02, 1] }}
              transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
              className="hidden xl:flex items-center px-3.5 py-1.5 border border-brand-blue/50 text-brand-blue font-black text-[10px] rounded-full hover:bg-brand-blue/10 transition-all active:scale-95 tracking-widest uppercase whitespace-nowrap"
            >
              Clube
            </motion.a>
            ...
            <Link
              to="/biblioteca"
              className="flex items-center px-4 py-1.5 xl:px-5 xl:py-2 bg-brand-blue text-bg-deep font-black text-[10px] xl:text-xs rounded-full hover:bg-white transition-all active:scale-95 tracking-widest uppercase whitespace-nowrap shadow-lg shadow-brand-blue/25"
            >
              Acessar
            </Link>
          </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden absolute top-[68px] lg:top-20 left-0 right-0 border-t border-white/[0.08]"
            ...
            <div className="max-w-7xl mx-auto px-4 py-5 flex flex-col gap-1">
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-1">
                {[
                  ...
                  { name: 'Diagramas', path: '/metodo',     delay: 0.05 },
                  { name: 'Tutoriais', path: '/tutoriais',  delay: 0.10 },
                  { name: 'Devocional',path: '/devocional', delay: 0.15 },
                  { name: 'Livraria',  path: '/livraria',   delay: 0.20, active: isLibrariaActive },
                ].map(link => {
                  const active = link.active ?? isActive(link.path);
                  return (
                    <motion.div key={link.name} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: link.delay }}>
                      <Link
